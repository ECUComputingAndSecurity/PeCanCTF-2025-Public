...
def products_page():
    ...
    search = form_data['search'] if "search" in form_data else None

    try:
        result = db.session.execute(text(f"SELECT * FROM products WHERE title LIKE '{search}'"))
    except OperationalError as e:
        black_list_later(request.remote_addr) # >:) Trying to attack me heh ? LOL !!!
        raise e
